...
final class ReplayWriter: @unchecked Sendable {
    ...
    private func configureOnQueue(
        ...
        if videoMode == .pixelBufferEncode {
            ...
            let estimatedBitrate = targetBitrateMbps(
    ...
    private func targetBitrateMbps(for quality: QualityPreset, videoSize: CGSize, frameRate: Int)
        -> Double
    {
    ...
    private func appendVideoOnQueue(_ sampleBuffer: CMSampleBuffer) {
        ...
        if CMSampleBufferGetImageBuffer(sampleBuffer) == nil, !loggedNoPixelBufferFormat {
            if let format = CMSampleBufferGetFormatDescription(sampleBuffer) {
                ...
                Log.info(
                    "ReplayWriter.appendVideo no CVPixelBuffer. mediaSubType:",
        ...
        if reconfigureForVideoModeIfNeeded(
            desiredMode: desiredMode, pixelBuffer: pixelBufferForMode)
        {
            return
        }
    ...
    private func reconfigureForVideoModeIfNeeded(
        ...
        reconfigureCount += 1
        Log.info(
            "⚠️ ReplayWriter.appendVideo reconfigure #\(reconfigureCount) for mode:", desiredMode)
    ...
    private func reconfigureForBufferSizeIfNeeded(pixelBuffer: CVPixelBuffer) -> Bool {
        ...
        reconfigureCount += 1
        Log.info(
            "⚠️ ReplayWriter.appendVideo reconfigure #\(reconfigureCount) for buffer size:", width,
            "x", height)
        guard let outputURL = outputURL else { return true }
    ...
    private func appendVideoSample(
        ...
        if !loggedFirstVideoBuffer, let pixelBuffer = CMSampleBufferGetImageBuffer(adjustedSample) {
            ...
            Log.info(
                "ReplayWriter.appendVideo first buffer:", bufferWidth, "x", bufferHeight, "format:",
                String(format: "0x%08X", format))
        ...
        if lastVideoPTS.isValid, pts < lastVideoPTS {
            Log.info(
                "ReplayWriter.appendVideo non-monotonic PTS. last:", lastVideoPTS.seconds, "now:",
                pts.seconds)
        }
        lastVideoPTS = pts
        ...
        if videoInput.isReadyForMoreMediaData {
            ...
            {
                Log.info(
                    "ReplayWriter.appendVideo backpressure drop count:", videoBackpressureDrops)
            }
        }
    }
    ...
    private func fillAudioGapIfNeeded(
        ...
        guard let format = audioFormatDescription,
            ...
            Log.info(
                "ReplayWriter.appendAudio gap:", gap.seconds * 1000,
    ...
    private func audioDurationForSample(_ sampleBuffer: CMSampleBuffer) -> CMTime? {
        ...
        if numSamples > 0, let audioSampleRate, audioSampleRate > 0 {
            return CMTime(
                value: CMTimeValue(numSamples), timescale: CMTimeScale(audioSampleRate.rounded()))
        }
        return nil
    }
    ...
    private func flushPendingAudioSamples(audioInput: AVAssetWriterInput, writer: AVAssetWriter) {
        ...
        if firstAudioPTS.isValid, firstVideoPTS.isValid, firstAudioPTS < firstVideoPTS {
            ...
            Log.info(
                "ReplayWriter: audio-video offset:", audioVideoOffset.seconds * 1000,
    ...
    private func flushPendingVideoSamples(writer: AVAssetWriter, videoInput: AVAssetWriterInput) {
        ...
        if appendedCount > 0 || droppedCount > 0 {
            Log.info(
                "ReplayWriter: flushed", appendedCount, "pending video samples, dropped:",
                droppedCount)
        }
    }
    ...
    private func logWriterError(_ label: String, _ error: Error?) {
        if let error {
            let nsError = error as NSError
            AppLog.error(
                .writer, label, "domain:", nsError.domain, "code:", nsError.code, "userInfo:",
                nsError.userInfo)
        } else {
    ...
    private func logAudioFormat(_ sampleBuffer: CMSampleBuffer) {
        ...
        audioASBD = asbd
        Log.info(
            "ReplayWriter.appendAudio ASBD: sr:", asbd.mSampleRate,
